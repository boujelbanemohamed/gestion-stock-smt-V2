...
# Fichier: fix-no-tailwind.sh
# Description: Script pour corriger l'application en désactivant Tailwind CSS

# --- Configuration ---
APP_DIR="/var/www/stock-management"
PM2_APP_NAME="stock-management-app"
LOG_FILE="$APP_DIR/fix_no_tailwind.log"
...
# --- Début du script ---
log_message "--- Démarrage de la correction sans Tailwind CSS ---"
...
# 4. Installation des dépendances sans Tailwind
log_message "4. Installation des dépendances sans Tailwind..."
npm cache clean --force
npm install --force
log_message "✓ Dépendances installées"

# 5. Installation spécifique de Prisma
log_message "5. Installation spécifique de Prisma..."
...
# 9. Désactiver Tailwind CSS dans next.config.mjs
log_message "9. Désactivation de Tailwind CSS..."
if [ -f "next.config.mjs" ]; then
  # Créer une sauvegarde
  cp next.config.mjs next.config.mjs.backup
  
  # Modifier le fichier pour désactiver Tailwind
...
EOF
  log_message "✓ Tailwind CSS désactivé dans next.config.mjs"
else
  log_message "⚠ Fichier next.config.mjs non trouvé"
fi

# 10. Modifier le fichier CSS principal pour utiliser du CSS simple
log_message "10. Modification du fichier CSS principal..."
if [ -f "app/globals.css" ]; then
  # Créer une sauvegarde
  cp app/globals.css app/globals.css.backup
  
  # Remplacer par du CSS simple
...
EOF
  log_message "✓ Fichier CSS modifié avec du CSS simple"
else
  log_message "⚠ Fichier app/globals.css non trouvé"
fi

# 11. Construction de l'application
log_message "11. Construction de l'application..."
npm run build
if [ $? -eq 0 ]; then
...
# 12. Vérifier que le dossier .next existe
log_message "12. Vérification du dossier .next..."
if [ -d ".next" ]; then
...
log_message "13. Redémarrage de l'application..."
...
# 17. Tester l'API stats
log_message "17. Test de l'API stats..."
STATS_RESPONSE=$(curl -s "$APP_URL/api/stats")
log_message "Réponse API Stats: $STATS_RESPONSE"

# 18. Vérifier si les nouveaux KPIs sont présents
...
# 20. Créer un script de test final
log_message "20. Création d'un script de test final..."
cat << 'EOF' > test-no-tailwind.sh
#!/bin/bash
...
chmod +x test-no-tailwind.sh
log_message "✓ Script test-no-tailwind.sh créé"

log_message "--- Correction sans Tailwind CSS terminée ---"
log_message "Utilisez './test-no-tailwind.sh' pour tester l'application"
