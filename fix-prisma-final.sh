...
# Fichier: fix-prisma-final.sh
# Description: Script pour corriger définitivement Prisma et les KPIs

# --- Configuration ---
APP_DIR="/var/www/stock-management"
PM2_APP_NAME="stock-management-app"
LOG_FILE="$APP_DIR/fix_prisma_final.log"
...
# --- Début du script ---
log_message "--- Démarrage de la correction finale de Prisma ---"
...
log_message "4. Installation des dépendances..."
npm cache clean --force
npm install --force
log_message "✓ Dépendances installées"

# 5. Installation spécifique de Prisma Client
log_message "5. Installation spécifique de Prisma Client..."
...
# 7. Vérifier que Prisma Client est installé
log_message "7. Vérification de l'installation de Prisma Client..."
if [ -d "node_modules/@prisma/client" ]; then
  log_message "✓ Prisma Client trouvé dans node_modules"
  ls -la node_modules/@prisma/client/ | head -5
else
  error_exit "Prisma Client non trouvé dans node_modules"
fi
...
# 17. Vérifier si les nouveaux KPIs sont présents
...
cat << 'EOF' > test-prisma-final.sh
#!/bin/bash
echo "Test final de Prisma et des KPIs..."
...
chmod +x test-prisma-final.sh
log_message "✓ Script test-prisma-final.sh créé"

log_message "--- Correction finale de Prisma terminée ---"
log_message "Utilisez './test-prisma-final.sh' pour tester l'application"
